...
- **Session Security & Auto-Logout**: Automated session management with "remember me" functionality, inactivity timeouts, and warning pop-ups before logout. React Error Boundary wraps the entire app to prevent white-screen crashes. `requireAdmin` middleware enforces `isSuperadmin` check. API error responses are sanitized to not leak internal details. Route params validated with NaN guards. **Forced Password Change**: Users created with temporary passwords (via group apply or admin) have `mustChangePassword: true` on their `users` record. A server-side middleware blocks all non-auth API calls until the password is changed. The `ForcePasswordChange` modal (non-dismissable) appears on login, requiring the user to set a new password (8+ chars). The flag is cleared in both `updateUserPassword` and the session cache when the password is changed.
