...
export default function Advisors() {
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/advisors', { credentials: 'include' }).then(r => r.ok ? r.json() : []),
      fetch('/api/universities', { credentials: 'include' }).then(r => r.ok ? r.json() : []),
    ]).then(([advisorsData, universitiesData]) => {
  ...
  const filteredAdvisors = universityFilter === 'all'
    ? advisors
    : advisors.filter(a => a.universityId === universityFilter);
            ...
            >
              <Card className="h-full" data-testid={`card-advisor-${advisor.id}`}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button variant="outline" size="sm" className="w-full" data-testid={`button-connect-${advisor.id}`}>
