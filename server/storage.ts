...
export class DatabaseStorage implements IStorage {
  ...
  // AI Next Steps methods
  async getNextSteps(ideaId: string): Promise<any[]> {
    const steps = await db.select()
      .from(schema.ideaNextSteps)
      .where(eq(schema.ideaNextSteps.ideaId, ideaId))
      .orderBy(desc(schema.ideaNextSteps.createdAt));
    
    return steps;
  }

  async generateNextSteps(ideaId: string, idea: any): Promise<any[]> {
    // Delete existing steps
    await db.delete(schema.ideaNextSteps).where(eq(schema.ideaNextSteps.ideaId, ideaId));
    
    const steps: any[] = [];
    const stage = idea.stage?.toLowerCase() || 'idea_posted';

    // Stage-specific recommendations
    if (stage.includes('idea_posted') || stage.includes('1')) {
      steps.push({
    ...
    if (stage.includes('find_advisors') || stage.includes('2')) {
      steps.push({
        id: crypto.randomUUID(),
        ideaId,
    ...
    for (const step of steps.slice(0, 5)) {
      const [inserted] = await db.insert(schema.ideaNextSteps).values(step).returning();
  ...
  async updateNextStep(ideaId: string, stepId: string, completed: boolean): Promise<void> {
    await db.update(schema.ideaNextSteps)
      .set({ completed, updatedAt: new Date() })
      .where(and(
        eq(schema.ideaNextSteps.id, stepId),
        eq(schema.ideaNextSteps.ideaId, ideaId)
      ));
  }
}
